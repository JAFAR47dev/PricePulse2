...
async def notifications_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "notif_settings":
        ...
        # format: notif_alert_strat_STRAT_COIN
        parts = data.replace("notif_alert_strat_", "").rsplit("_", 3)
        # strat keys are like strat_1, so we need to be careful
        # format stored as: notif_alert_strat_{strat_key}|{coin}
