...
EXCLUDED_COMMANDS = {
    ...
    "portfolio", "portfoliotarget", "portfoliolimit", "prediction", "edit", "stats", "setplan", "prolist", "calc", "dc"
}
...
# Load top 100 coins (symbol → id mapping)
with open("utils/top_100_coins.json") as f:
    top_100 = json.load(f)
symbol_to_id = {coin["symbol"]: coin["id"] for coin in top_100}


async def calc_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        if len(context.args) != 2:
            return await update.message.reply_text("Usage: /calc [coin] [amount]")

        symbol = context.args[0].lower()
        amount = float(context.args[1])

        if symbol not in symbol_to_id:
            return await update.message.reply_text("❌ Coin not supported. Only top 100 coins are allowed.")
        ...
        text = (
            ...
            f"💰 *{amount} {symbol.upper()}* = *${total:,.2f}*"
        )

        await update.message.reply_text(text, parse_mode=ParseMode.MARKDOWN)

    except ValueError:
        await update.message.reply_text("❌ Invalid amount. Use a number like `/calc btc 1.5`")
    except Exception as e:
