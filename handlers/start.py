...
def main_keyboard() -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [
            InlineKeyboardButton("📈 BTC Chart", callback_data="start_levels"),
...
# ============================================================================
"""
Replaces handle_start_levels with handle_start_chart.

Instead of fetching levels data and rendering a text preview,
this handler generates and sends the BTC 1h TradingView chart
directly — the same output as /c btc 1h but triggered from
the start screen button.

format_levels_preview and _levels_engine are removed entirely.
LevelsEngine is still available via /levels btc 1h as normal.
"""
...
async def handle_start_levels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Kept as handle_start_levels so your callback registration doesn't change.
    Now sends the BTC 1h chart instead of a levels text preview.
    """
    query = update.callback_query
    await query.answer("Loading BTC chart…")

    loading = await query.message.reply_text(
        "⏳ *Generating BTC 1h chart…*",
        parse_mode="Markdown",
    )

    try:
        image_bytes = await generate_chart_image("BTCUSDT", "1h", context)

        if not image_bytes:
            raise ValueError("Chart generation returned no image")
        ...
        await query.message.reply_photo(
            ...
            caption = (
                "📈 *BTC* — 1h Chart\n\n"
                "_Type `/c btc 4h` for other timeframes_\n"
                "_or `/c eth 1h`, `/c sol 1d` for other coins_"
            ),
            parse_mode = "Markdown",
        )
        ...
        await loading.edit_text(
            "⚠️ Chart unavailable right now.\n\n"
            "Type `/c btc 1h` to load it directly.",
            parse_mode = "Markdown",
        )
