...
async def setup_alert_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle set alert button - IMPROVED VERSION"""
    ...
    try:
        ...
        if not setup_data:
            await query.answer(
                "❌ Setup data expired.\n\n"
                "Please run /setup again to get fresh data.",
                show_alert=True
            )
            return
        ...
        # Build alert message based on direction
        if direction == "BULLISH":
            recommended_alert = f"/set {symbol} above {entry_low:.2f}"
            explanation = "Alert when price reaches bullish entry zone"
        elif direction == "BEARISH":
            recommended_alert = f"/set {symbol} below {entry_high:.2f}"
            explanation = "Alert when price reaches bearish entry zone"
        else:
            recommended_alert = f"/set {symbol} above {current_price:.2f}"
            explanation = "Set alert at your preferred price"
        ...
        message = (
            ...
            f"`{recommended_alert}`\n"
            f"_{explanation}_\n\n"
            f"**Alternative alerts:**\n"
            f"• Entry low: `/set {symbol} above {entry_low:.2f}`\n"
            f"• Entry high: `/set {symbol} above {entry_high:.2f}`\n"
            f"• Custom: `/set {symbol} above [PRICE]`\n\n"
            f"💡 Copy the command and send it!"
        )
...
async def setup_charts_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        message = (
            f"📊 **Chart View for {symbol}**\n\n"
            f"Use: `/c {symbol} {timeframe}`\n\n"
            f"This will show you:\n"
            f"• TradingView chart link\n"
            f"• Current price action\n\n"
            f"💡 Analyze the chart alongside the setup!"
        )
...
async def setup_refresh_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await query.answer("🔄 Refreshing analysis...", show_alert=False)
        ...
        # Simulate re-running the command
        context.args = [symbol, timeframe]
        
        # Show loading
        await query.edit_message_text(
            f"🔄 Refreshing {symbol} {timeframe} analysis...\n\n"
        ...
        if not setup_data:
            await query.edit_message_text(
                ...
                f"Try running: `/setup {symbol} {timeframe}`",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        await query.edit_message_text(
            "❌ Refresh failed. Try running `/setup [SYMBOL] [TIMEFRAME]` again.",
            parse_mode=ParseMode.MARKDOWN
        )
